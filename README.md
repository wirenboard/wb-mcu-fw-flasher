...
Во всех наших устройствах реализован механизм загрузчика прошивок
...
Bootloader реализует следующий функционал:
  ...
  - сброс Modbus-настроек для основной программы в значения по умолчанию
  - сброс всех настроек и калибровок устройства в значения по умолчанию.

**В режиме bootloader'а коммуникационные параметры Modbus фиксированы и не зависят
от их настроек на устройстве: 9600 8N2.** Начиная с версии загрузчика 1.3.0 имеется
возможность перейти из прошивки в загрузчик с сохранением коммуникационных параметров.
Выбор Modbus-адреса устройства описан ниже.
...
Устройство переходит в режим bootloader'а:
  ...
  - при работе основной программы в holding-регистр 129 (0x81) была
    ...
    секунд. Коммуникационные параметры Modbus устанавливаются в
    значения по умолчанию (9600 8N2), адрес не меняется;
...
В режиме bootloader'а индикатор Status мигает 1 раз в 2 секунды.
...
При первой записи прошивки на заводе в устройство записывается сигнатура
— наименование аппаратного типа устройства. При перезаписи прошивки
текущая сигнатура сравнивается с сигнатурой из информационного
...
1.  Основной адрес для прошивки — адрес, хранящийся в памяти устройства (EEPROM
    или FlashFS), то есть это стандартный адрес, на который устройство отвечает
    из основной программы.
2.  Прошивать устройства можно по широковещательному адресу 0. Это менее
    предпочтительный способ при рекомендации его пользователям.
...
* сбрасывает Modbus-настройки (адрес и коммуникационные параметры) в заводские (1, 9600N2)
...
*Все нижесказанное актуально только в том случае, если у устройства есть EEPROM. При отсутствии после указанных действий ничего происходить не будет*
...
* сбрасывает Modbus-настройки в заводские (1, 9600N2)
* при успешном выполнении посылает стандартный ответ Modbus
...
```bash
wb-mcu-fw-flasher -j -d /dev/ttyRS485-1 -a 1 -f MR6C_GD32E230K8_NO_EEPROM__1.21.5_feature_fw818-remove-eeprom_7f1a746.wbfw
```

Успешный процесс прошивки выглядит следующим образом:

```
  wb-mcu-fw-flasher -j -d /dev/ttyRS485-1 -a 1 -f WMR6C_GD32E230K8_NO_EEPROM__1.21.5_feature_fw818-remove-eeprom_7f1a746.wbfw
  /dev/ttyRS485-2 opened successfully.
  Send jump to bootloader command and wait 2 seconds...
  Ok, device will jump to bootloader.
  MR6C_GD32E230K8_NO_EEPROM__1.21.5_feature_fw818-remove-eeprom_7f1a746 wbfw opened successfully, size 24648 bytes
...
использовать wb-mcu-fw-flasher версии 1.0.5 и выше с флагом -B
...
программы, так как пользователям не нужно его использовать). Режим
приема прошивки на скорости 115200 активен когда нет сигнатуры -
пустая память с завода.
