...
| 1005    | 1     | holding | single write   | Сброс всех настроек, хранящихся во FlashFS, включая калибровки и настройки связи | С версии 1.5.0 |
...
## Wiren Board Component Firmwares

Устройство Wiren Board кроме своей основной прошивки может иметь до **8** дополнительных прошивок (component firmware).

Вывод информации о наличии и версиях дополнительных прошивок реализован в wb-mcu-fw-flasher начиная с версии **1.6.0** с помощью опции `--get-device-info`:

```bash
root@wirenboard-A7XCVEGT:~# ./wb-mcu-fw-flasher -d /dev/ttyRS485-1 -a1 -b115200 --get-device-info
/dev/ttyRS485-1 opened successfully.
Bootloader version: 1.5.5
Firmware version: 4.34.1
Firmware signature (fw-sig): msw5Ge
Download firmwares: https://fw-releases.wirenboard.com/?prefix=fw/by-signature/msw5Ge/

Component firmwares:
  Component 1:
    Model: CO2 Sensor NS8
    Version: MF1.03D
    Signature: co2_sens_ns8
    Download: https://fw-releases.wirenboard.com/?prefix=fw/by-signature/co2_sens_ns8/
```

### Регистры с информацией о дополнительных прошивках

| Адрес           | Длина | Тип            | Описание
|-----------------|-------|----------------|-----------------------------|
| 0xFE80          | 8     | discrete input | Флаги наличия дополнительных прошивок
| 0xFD00 + N * 48 | 20    | input          | Модель дополнительного компонента в ячейке N (один символ в регистре)
| 0xFD14 + N * 48 | 12    | input          | Сигнатура прошивки дополнительного компонента в ячейке N (один символ в регистре)
| 0xFD20 + N * 48 | 16    | input          | Версия прошивки дополнительного компонента в ячейке N (один символ в регистре)
| 0x1000          | 16    | holding        | info-блок прошивки
| 0x2000          | 68    | holding        | data-блоки прошивки


### Проверка наличия дополнительных прошивок

Чтобы проверить, есть ли у устройства дополнительная прошивка, нужно прочитать 8 discrete input регистров с адреса `0xFF80`:

```
root@wirenboard-A7XCVEGT:~# modbus_client -mrtu -pnone -s2 /dev/ttyRS485-1 -a1 -t2 -r0xFE80 -c8
SUCCESS: read 8 of elements:
        Data: 0x01 0x00 0x00 0x00 0x00 0x00 0x00 0x00

# в данном случае есть одна дополнительная прошивка в ячейке 0
```

Устройство может ответить:
- ошибкой ILLEGAL_DATA_ADDRESS - это значит, что оно не поддерживает дополнительные прошивки
- ошибкой BUSY - это значит, устройство ещё не готово к работе и нужно повторять запрос до тех пор, пока не будет получен ответ
- 8 флагами (0 или 1) - это значит, что у устройства есть или нет соответствующая дополнительная прошивка
- если вклчюен режим непрерывного чтения регистров и регистры не существуют, то устройство вернет 8 нулей - это значит, что дополнительных прошивок нет

### Версия и сигнатура дополнительных прошивок

Для каждой дополнительной прошивки можно прочитать её модель (человекочитаемое название), сигнатуру и версию. Пример для дополнительной прошивки в ячейке 0:

```
root@wirenboard-A7XCVEGT:~# echo -e $(modbus_client -mrtu -pnone -s2 /dev/ttyRS485-1 -a1 -t3 -r0xfd00 -c20 | grep Data | sed -e 's/.*Data://' -e 's/ 0x00/\\x/g')
CO2 Sensor NS8
root@wirenboard-A7XCVEGT:~# echo -e $(modbus_client -mrtu -pnone -s2 /dev/ttyRS485-1 -a1 -t3 -r0xfd20 -c16 | grep Data | sed -e 's/.*Data://' -e 's/ 0x00/\\x/g')
MF1.03D
root@wirenboard-A7XCVEGT:~# echo -e $(modbus_client -mrtu -pnone -s2 /dev/ttyRS485-1 -a1 -t3 -r0xfd14 -c12 | grep Data | sed -e 's/.*Data://' -e 's/ 0x00/\\x/g')
co2_sens_ns8
```

### Прошивка дополнительных прошивок

Вся работа с дополнительными прошивками происходит из основной прошивки устройства, **переход в загрузчик не требуется**.

Файлы дополнительных прошивок находятся там же, где и файлы основных прошивок: https://fw-releases.wirenboard.com/?prefix=fw/by-signature/signature/main.

Загрузка дополнительной прошивки происходит точно так же, как и загрузка основной прошивки, но без перехода в загрузчик. Пример команды:
```
root@wirenboard-A7XCVEGT:~# wb-mcu-fw-flasher -d /dev/ttyRS485-1 -B115200 -a1 -f co2_sens_ns8__MF1.03D.compfw
/dev/ttyRS485-1 opened successfully.
co2_sens_ns8__MF1.03D.compfw opened successfully, size 29544 bytes

Sending info block... OK

Sending data block 217 of 217... OK.

All done!
```

Если устройство работает на скорости, отличной от 9600, то нужно указать её с помощью флага `-B`. Обратите внимание, что флаг `-B` используется для задания скорости при обновлении прошивки, в отличие от флага `-b`, который используется для задания скорости при общении с основной прошивкой устройства (например, для перехода в загрузчик или получения информации об устройстве).
