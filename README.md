...
от настроек на устройстве: 9600 8N2.** Начиная с версии загрузчика 1.3.0 имеется
...
Устройство переходит в режим загрузчика:
  - если основная программа присутствует - при каждом включении питания
    загрузчик активен в течение 2 секунд
  - если основная программа отсутсвует в памяти (например, был сбой при
    обновлении прошивки) - в этом случае загручик активен постоянно
  - при работе основной программы в holding-регистр 129 (0x81) была
    ...
    устройство перезагружается и остается в режиме загрузчика 120
...
Прошивка основной программы выполняется загрузчиком, который получает
...
Modbus-адрес для загрузки прошивки в режиме загрузчика следует выбирать
следующим образом:

1.  Основной адрес для прошивки — адрес, хранящийся в памяти устройства (EEPROM
    или FlashFS в зависимости от устройства), то есть это стандартный адрес,
    на который устройство отвечает из основной программы.
...
В режиме загрузчика поддерживается ограниченный набор Modbus-команд.
...
Выберите подходящую прошивку, например,
mr6cG__1.17.7_master_52bdb26.wbfw
...
Bootloader позволяет загружать прошивку на устройства по Modbus-адресу
0. Для этого устройство должно быть единственным устройством на шине и
находиться в режиме bootloader'a.
